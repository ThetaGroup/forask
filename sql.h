...
#define CREATE_TASK_TABLE_SQL "create table task(id integer primary key autoincrement,title varchar(128),content varchar(1024),state varchar(2),areaId integer,createTime datetime,finishTime datetime);"
...
#define INSERT_TASK_SQL "insert into task (title,content,state,areaId,createTime) values ('%s','%s','%s','%ld',datetime('now'));"

#define UPDATE_SWITHCED_AREA_SQL "update config set attr='%s' where name='current_area';"
#define UPDATE_TASK_STATE_SQL "update task set state='%s',finishTime=datetime('now') where  id='%ld';"
